...
class HostListView(LoginRequiredMixin, ListView):
    login_url = 'login'
    model = Host
    template_name = 'events/host.html'
    context_object_name = 'host'


class NewHostView(LoginRequiredMixin, CreateView):
    login_url = 'login'
    model = Host
    fields = ['name', 'code', 'image', 'priority', 'status']
    template_name = 'events/new_host.html'
...
#     login_url = 'login'
#     model = Host
#     fields = ['name', 'code', 'priority', 'status']
...
class EventUpdateView(LoginRequiredMixin, UpdateView):
    login_url = 'login'
    model = Event
    fields = ['host', 'name', 'uid', 'description', 'scheduled_status', 'venue', 'start_date', 'end_date', 'points', 'maximum_attende', 'status']
...
def search_host(request):
    if request.method == 'POST':
       ...
       host = Host.objects.filter(name__icontains=data)
       context = {
           'host': host
       }
       return render(request, 'events/host.html', context)
    return render(request, 'events/host.html')
